...
! Loop indices
INTEGER(KIND=JPIM) :: JLON, JLEV, JBAND, JAER, ISW
...
DO JLON = KIDIA,KFDIA
  CALL YLCLOUD%SET_OVERLAP_PARAM_FIX(YLCLOUD, THERMODYNAMICS,&
      &                       ZDECORR_LEN_KM(JLON)*1000.0_JPRB,&
      &                       STARTCOL=JLON, ENDCOL=JLON)
ENDDO
...
IF (YRERAD%NAERMACC == 1) THEN
  ...
  IF (YRERAD%NAERMACC == 1) THEN
    ...
    IF (TROP_BG_AER_MASS_EXT > 0.0_JPRB) THEN
      AEROSOL%MIXING_RATIO(KIDIA:KFDIA,1:KLEV,ITYPE_TROP_BG_AER)&
           &  = AEROSOL%MIXING_RATIO(KIDIA:KFDIA,1:KLEV,ITYPE_TROP_BG_AER)&
           &  + PAEROSOL_OLD(KIDIA:KFDIA,1,1:KLEV)&
           &  / (ZLAYER_MASS(KIDIA:KFDIA,1:KLEV) * TROP_BG_AER_MASS_EXT)
    ENDIF
    IF (STRAT_BG_AER_MASS_EXT > 0.0_JPRB) THEN
      AEROSOL%MIXING_RATIO(KIDIA:KFDIA,1:KLEV,ITYPE_STRAT_BG_AER)&
           &  = AEROSOL%MIXING_RATIO(KIDIA:KFDIA,1:KLEV,ITYPE_STRAT_BG_AER)&
           &  + PAEROSOL_OLD(KIDIA:KFDIA,6,1:KLEV)&
           &  / (ZLAYER_MASS(KIDIA:KFDIA,1:KLEV) * STRAT_BG_AER_MASS_EXT)
    ENDIF
  ENDIF
ELSE
...
CALL GAS%PUT_GAS_JPRM(GAS, IH2O,    IMASSMIXINGRATIO, PQ)
CALL GAS%PUT_GAS_JPRM(GAS, ICO2,    IMASSMIXINGRATIO, PCO2)
CALL GAS%PUT_GAS_JPRM(GAS, ICH4,    IMASSMIXINGRATIO, PCH4)
CALL GAS%PUT_GAS_JPRM(GAS, IN2O,    IMASSMIXINGRATIO, PN2O)
CALL GAS%PUT_GAS_JPRM(GAS, ICFC11,  IMASSMIXINGRATIO, PCFC11)
CALL GAS%PUT_GAS_JPRM(GAS, ICFC12,  IMASSMIXINGRATIO, PCFC12)
CALL GAS%PUT_GAS_JPRM(GAS, IHCFC22, IMASSMIXINGRATIO, PHCFC22)
CALL GAS%PUT_GAS_JPRM(GAS, ICCL4,   IMASSMIXINGRATIO, PCCL4)
CALL GAS%PUT_GAS_JPRM(GAS, IO3,     IMASSMIXINGRATIO, PO3)
#else
CALL GAS%PUT_GAS_JPRD(GAS, IH2O,    IMASSMIXINGRATIO, PQ)
CALL GAS%PUT_GAS_JPRD(GAS, ICO2,    IMASSMIXINGRATIO, PCO2)
CALL GAS%PUT_GAS_JPRD(GAS, ICH4,    IMASSMIXINGRATIO, PCH4)
CALL GAS%PUT_GAS_JPRD(GAS, IN2O,    IMASSMIXINGRATIO, PN2O)
CALL GAS%PUT_GAS_JPRD(GAS, ICFC11,  IMASSMIXINGRATIO, PCFC11)
CALL GAS%PUT_GAS_JPRD(GAS, ICFC12,  IMASSMIXINGRATIO, PCFC12)
CALL GAS%PUT_GAS_JPRD(GAS, IHCFC22, IMASSMIXINGRATIO, PHCFC22)
CALL GAS%PUT_GAS_JPRD(GAS, ICCL4,   IMASSMIXINGRATIO, PCCL4)
CALL GAS%PUT_GAS_JPRD(GAS, IO3,     IMASSMIXINGRATIO, PO3)
#endif
CALL GAS%PUT_WELL_MIXED(GAS, IO2, IVOLUMEMIXINGRATIO, 0.20944_JPRB)
...
IF (RAD_CONFIG%DO_LW .AND. YRERAD%LAPPROXLWUPDATE) THEN
  PLWDERIVATIVE(KIDIA:KFDIA,:) = FLUX%LW_DERIVATIVES(KIDIA:KFDIA,:)
ENDIF

! Store the shortwave downwelling fluxes in each albedo band
IF (RAD_CONFIG%DO_SW .AND. YRERAD%LAPPROXSWUPDATE) THEN
  DO ISW=1,YRADIATION%YRERAD%NSW
    DO JLON=KIDIA,KFDIA
      PSWDIFFUSEBAND(JLON,ISW) = FLUX%SW_DN_DIFFUSE_SURF_CANOPY(ISW,JLON)
      PSWDIRECTBAND (JLON,ISW) = FLUX%SW_DN_DIRECT_SURF_CANOPY (ISW,JLON)
    END DO
  END DO
ENDIF
